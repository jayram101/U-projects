...
bioj.education.display = function() {
    ...
    if (j >= 0) {
        ...
        for (school in bioj.education.schools) {
            var formattedSchoolName = HTMLschoolName.replace("%url%", bioj.education.schools[school].url);
            formattedschoolName = HTMLschoolName.replace("%data%", bioj.education.schools[school].name).concat(HTMLschoolDegree.replace("%data%", bioj.education.schools[school].degree));
